...
export async function POST(
  ...
  try {
    ...
    const closedEvent = await EventService.closeRegistrations(params.eventId);

    return NextResponse.json({
      success: true,
      data: { event: closedEvent },
      message: 'Registrations closed successfully',
    });
  } catch (error) {
    console.error('Close event error:', error);
    const message = error instanceof Error ? error.message : "Failed to close registrations"
    return NextResponse.json(
      {
        success: false,
        error: {
