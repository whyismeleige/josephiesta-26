...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  // --- UPDATED REFS FOR HAMBURGER ---
  const iconRef = useRef<HTMLSpanElement | null>(null);
  const topBarRef = useRef<SVGPathElement | null>(null);
  const middleBarRef = useRef<SVGPathElement | null>(null);
  const bottomBarRef = useRef<SVGPathElement | null>(null);
  // ----------------------------------
  ...
  // --- UPDATED ANIMATION FOR HAMBURGER TO X ---
  const animateIcon = useCallback((opening: boolean) => {
    const icon = iconRef.current;
    const top = topBarRef.current;
    const middle = middleBarRef.current;
    const bottom = bottomBarRef.current;
    
    if (!icon || !top || !middle || !bottom) return;

    spinTweenRef.current?.kill();
    spinTweenRef.current = gsap.timeline({ defaults: { duration: 0.4, ease: 'power3.inOut' } });

    if (opening) {
      // Turn into X
      spinTweenRef.current
        .to(middle, { opacity: 0, duration: 0.2 }, 0)
        .to(top, { y: 6, rotate: 45, transformOrigin: "center" }, 0)
        .to(bottom, { y: -6, rotate: -45, transformOrigin: "center" }, 0)
        .to(icon, { rotate: 90 }, 0);
    } else {
      // Turn back to Hamburger
      spinTweenRef.current
        .to(middle, { opacity: 1, duration: 0.2 }, 0.1)
        .to(top, { y: 0, rotate: 0, transformOrigin: "center" }, 0)
        .to(bottom, { y: 0, rotate: 0, transformOrigin: "center" }, 0)
        .to(icon, { rotate: 0 }, 0);
    }
  }, []);
  ...
  const toggleMenu = useCallback(() => {
    ...
    animateIcon(target);
    animateColor(target);
    // animateText(target); // Removed call
  ...
  const closeMenu = useCallback(() => {
    if (openRef.current) {
      ...
      animateIcon(false);
      animateColor(false);
      // animateText(false); // Removed call
    }
    ...
    >
      ...
      >
          ...
          >
            ...
            >
               <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path ref={topBarRef} d="M4 6H20" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                <path ref={middleBarRef} d="M4 12H20" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                <path ref={bottomBarRef} d="M4 18H20" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                </svg>
            </span>
        ...
        <aside
          ...
          ref={panelRef}
          className="staggered-menu-panel absolute top-0 right-0 h-full bg-white flex flex-col p-[6em 2em 2em 2em] overflow-y-auto z-10 backdrop-blur-[12px] pointer-events-auto"
...
/* Removed sm-toggle-text and sm-icon-line styles */
